...
Supersede the hand-managed per-agent fan-out (4b–4c) with a **Workflow-orchestrated `author → skeptic-verify` pipeline**: one `Workflow` per wave, `pipeline(candidates, author, verify)`, where the verifier defaults `is_consistent:false`, **re-reads the authored file on disk**, and refutes against a structured checklist (citation-correctness, no `depends_on:`, all `relates_to:`/`concepts:` resolve, discipline held). The verify stage is the registered **`darren-workflow:skeptic`** subagent (read-only; falls back to `agentType:'Explore'` if the registry has not loaded the type — a newly-added subagent needs a `/plugin` re-scan/update to register, **NOT merely a session restart**, because the `@local` plugin snapshots its agent set at install/scan time; see §4f). Reusable template: **`docs/research/planning/templates/intake-wave.workflow.template.js`**. Full method narrative + evidence: **`docs/research/connections/sahely-intake-arc-method-retrospective.md`**.
...
### 4f. Verify-stage checklist validation (the verifier's coverage is not self-validating)

The `darren-workflow:skeptic` verify stage (§4a) reduces citation/discipline error but **cannot audit its own coverage** — a green "all checks pass" is confidence proportional to the checklist's completeness, which the verifier could not judge (retrospective §5 risk (c)). Make that coverage testable from outside:

- **Run a deliberately-broken-fixture coverage suite.** Maintain a fixture set where each fixture is a clean bridge note injected with **exactly one** violation, one per checklist item the skeptic claims to catch (drifted/stale ADR citation; forbidden `depends_on:`; unresolved `relates_to:`; off-registry `concepts:` slug; `admit-candidate` row disposition; inline canon-admission; fabricated quote). Assert each fixture trips **its** check (`is_consistent:false` + the offender quoted), the **clean control passes**, and a **silent-pass guard** holds: a fixed-literal `EXPECTED` count compared to `M` (non-null verdicts), so a loop that under-ran (the zsh unquoted-`$var` ran-zero/ran-once bug noted in §4a) is flagged RED, not passed green. Re-run it whenever the checklist or the skeptic prompt changes. Reference implementation: `darren-workflow/agents/skeptic-coverage/` (runner + fixtures + a standing coverage report).
- **For multi-phase arcs, `skeptic-of-skeptics` (§15b) refutes the checklist RESULTS**, not only the downstream verdicts — it surfaces that coverage was sampled, not proven, unless a coverage suite was actually run.
- **PAIRED-DEPENDENCY gotcha.** The verify stage is bound to the skeptic by `VERIFIER_AGENT_TYPE` (`templates/intake-wave.workflow.template.js` ~line 109) ⇄ `darren-workflow/agents/skeptic.md`. Rename or remove either without the other and the stage **silently falls back to `agentType:'Explore'`** — read-only and behaviorally close, but NOT the registered checklist, and no error is raised. Treat the two as one unit. Documented in three places: (a) inline at the template line, (b) `agents/skeptic-coverage/README.md`, (c) here.
- **Registration mechanism.** A newly-added `darren-workflow/agents/*.md` is **not** picked up by a plain session restart — the `@local` plugin snapshots its agent set at install/scan time, so registering a new agent type needs a `/plugin` re-scan/update (disable+enable), after which `agentType:'darren-workflow:skeptic'` resolves. Until then the `'Explore'` fallback runs (embed the checklist in the verify prompt, since Explore lacks `skeptic.md` as its system prompt). See `feedback_darren_workflow_agent_registration.md`.
...
- **Topology determines mechanism.** Downstream-aligned siblings (IC, PM) propagate via alignment ADRs at write-time; peer instance-family repos (BKC, bregion, and bregion's sub-domain canons `bioregional-mapping` + `bioregional-economics` — all citing the upstream grammar canon, the sub-domains via bregion as *nearest peer* with Spore as transitive root per their ADR-0001s) close gaps via bridge notes at read-time. Choose the mechanism the relationship calls for. See `feedback_peer_instance_family_vs_downstream_aligned.md` + `feedback_upstream_downstream_canon_propagation.md`.
...
### 15a. Holistic-read sign-off gate (operationalizes judgment (a) — blocking)

A "zero new pressure / nothing-found / done" verdict produced by an atomized framing-note-only fan-out is a **false-negative-on-aggregate** risk: no single agent read the corpus as a whole, so a theme weak in every note but threshold-crossing in aggregate is systematically under-detected (retrospective §5 (a)). Before any phase declares that verdict — **especially right after a Workflow author→verify wave or a propagation sweep** — a **blocking holistic-read sign-off** is required:

- **Reader:** exactly **one** reader who reads the corpus *as a whole* — a human, or a single cross-corpus agent given the whole wave at once (NOT one-agent-per-note, which re-creates the blind spot).
- **Scope:** scan for cross-note term-usage drift, ≥5-term recurrence, weak-per-note/strong-in-aggregate themes, cross-note contradictions; verify the capstone's accuracy; spot-read 1–2 notes per wave end-to-end.
- **Sign-off:** fill the structured template at [`templates/holistic-read-signoff.template.md`](templates/holistic-read-signoff.template.md) (reader / date / corpus scope / reading method / aggregate signals / capstone-accuracy / verdict / confidence).
- **Checklistable hard item:** *holistic-read sign-off present, all fields filled, verdict + confidence recorded.* A missing or blank sign-off means "done" is **not earned**. The gate enforces the read's presence + structure; the verdict (UPHELD vs OVERTURNED) stays a judgment — do not automate it away.

### 15b. Skeptic-of-skeptics audit checklist (what the meta-auditor refutes)

When a multi-phase arc is declared done (§15), the `darren-workflow:skeptic-of-skeptics` audits, explicitly:

1. **Per-auditor checklist coverage** — was a checklist-coverage suite (§4f) actually run, or is "all checks pass" merely asserted? Coverage sampled ≠ coverage proven.
2. **Aggregation arithmetic** — do the per-surface auditor counts add up; is any item double-counted or silently dropped?
3. **`disposition-label ≠ deliverable-spec`** — a verdict-word ("framing-note-only", "decline-with-trigger", "admit") is a judgment about what is *owed*, NOT a missing deliverable. Refute any "missing" flag that is really a disposition.
4. **Silent-pass detection** — treat any silent-pass construct (a verification loop that ran zero/few iterations; `M ≠ EXPECTED` in a coverage run; an unquoted `$var` in a zsh loop) as a **coverage failure**, not a pass.

**Taxonomy (DG-T reconciliation).** The [retrospective §5](../connections/sahely-intake-arc-method-retrospective.md) lists **three** risks (a/b/c); the canonical framing is **two cannot-self-validate JUDGMENTS** — (a) the zero-pressure verdict (→ §15a holistic-read gate) and (c) verifier-checklist coverage (→ §4f coverage suite) — **plus one structural LIMIT**, (b) *shared ground-truth shares its errors* (author + verifier embed the same consistency-map). (b) is not a gap in verifying the work but a property of shared-input design, **mitigated by deriving the consistency-map from live canon at workflow-start** (§4a's Canon-facts phase), not by a human sign-off. The two method memories (`feedback_completeness_audit_skeptic_of_skeptics.md`, `feedback_workflow_orchestrated_intake.md`) carry this same 2-judgments-plus-1-structural-limit framing.

## Changelog

- **v4** (2026-05-31): Self-improving-loop institutionalization (Phase 3 of the self-querying / self-routing / self-improving arc). Adds **§4f** (verify-stage checklist validation — the skeptic's coverage made testable via a deliberately-broken-fixture suite with a fixed-literal `EXPECTED==M` silent-pass guard; the `VERIFIER_AGENT_TYPE` ⇄ `skeptic.md` paired-dependency gotcha; the corrected plugin-rescan registration mechanism), **§15a** (the holistic-read sign-off as a BLOCKING gate, with a fill-in template), and **§15b** (the skeptic-of-skeptics audit checklist + the DG-T taxonomy reconciliation: 2 cannot-self-validate judgments + 1 structural-limit). Body-only; frontmatter unchanged. Coverage-suite reference implementation: `darren-workflow/agents/skeptic-coverage/`. New template: `templates/holistic-read-signoff.template.md`. Harvested from the Phase-3 self-improving-loop build.
