...
- `archived` — doc preserved as historical record, intentionally out of rotation; no successor required (no `superseded_by:` field needed) and no implication of retirement from live canon (which `deprecated` carries). Canonical use case: external research papers brought into the corpus as reference material (e.g. `docs/research/external/`), historical planning docs retained for audit-trail, bridge notes whose source has moved on without a one-for-one replacement internal to this repo. Distinct from `deprecated` (retired from active canon use without successor) and from `superseded` (one-for-one replacement via `superseded_by:` field). Universal enum: `archived` is valid for all non-proposal doc_kinds (research / decision-record / pattern / spec / foundation / architecture / vision / protocol / operations / positioning / roadmap / connection / planning).
