...
R-claim format per `learning-field-intake-protocol.md`: `[target: <spore-doc-id>] [concept: <slug>]` where target grep-resolves in live frontmatter and concept resolves in concepts-p2p-wiki.yaml v20 (the version live at authoring-time; ADR-0084 subsequently admitted `route-graph` + `settlement-operator` in v21). Per the plan's concept-slug parking convention, candidate slugs not in v20 at authoring-time (`route-graph`, `settlement-operator`, `cost-function-composition` — first two now admitted via ADR-0084; `cost-function-composition` remains candidate per DECISION-BRIEF) appeared in body prose only — never inside R-claim `[concept:]` tags.
