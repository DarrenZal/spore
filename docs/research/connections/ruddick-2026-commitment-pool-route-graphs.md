...
| Settlement operator Σ | NEW — no Spore primitive captures settlement-as-institution-relative-operator; candidate vocab slug `settlement-operator` (NOT in v20; see §8 + DECISION-BRIEF) | vocab/glossary (CANDIDATE) | STRONG |
...
1. **`route-graph` glossary slug** (candidate slug; not yet in v20). Likely ADMIT under cluster-counting — names a clearly-operational object cited in Mehrling layered-claims tradition + Lamport state-machine specifications + Ruddick CPP + adjacent network-economics. β-evidence: ≥4 instance-families per ADR-0068 + Ruddick §5–§6 worked constructions. **DEFERRED** to operator-gated follow-on; see DECISION-BRIEF.
2. **`settlement-operator` glossary slug** (candidate slug; not yet in v20). Likely ADMIT under cluster-counting — named in ≥3 traditions (Lamport state-machine specifications use Σ-shaped operator implicitly; Mehrling layered-claims explicitly distinguish settlement from convertibility; Ruddick formalizes Σ as institution-relative). **DEFERRED** to follow-on.
