...
- **Extended via prose per ADR-0086 §Decision (2026-05-25; extend-via-prose precedent per ADR-0050 / ADR-0077; this ADR's frontmatter is UNCHANGED)** — `life-value-doctrine` (ADR-0086) admits the McMurtry immanent-ethics substrate (C4 R-Immanent-ethics from the Sahely corpus Phase 2 closure DECISION-BRIEF) as a cross-cutting doctrine sibling to `reproductive-commoning` (ADR-0002), `boundary-commoning` (ADR-0003), and `care-commoning` (ADR-0045). Structural-legitimacy here gains an evaluative substrate: legitimacy of coordination outcomes is readable through the life-value lens (range of being / doing / feeling / thinking enabled, per McMurtry's Life-Value Onto-Axiology) without requiring primitive-grammar extension. The authority-consequence coupling invariant established by this ADR continues to govern *what makes coordination authority legitimate at the structural layer*; life-value-doctrine adds an evaluative lens for *what coordination outcomes the legitimately-coupled authority serves*. The two operate in tandem, not in subsumption — structural-legitimacy as substrate-discipline, life-value-doctrine as evaluative-substrate. See ADR-0086 §Decision and `docs/research/connections/canon-framing-life-value-doctrine.md` for full doctrine articulation.
