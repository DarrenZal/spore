...
**Trap-shape vocabulary extension (per ADR-0085 admission 2026-05-23):** The substitution-trap mode takes canon-legible shape variants. ADR-0085 admits two derived-glossary slugs naming the canonical shapes:

- **`golden-calf-trap`** — simple substitution shape; canonical 5-stage: corrective distinction → captured → hardened → ritualized → selection-mechanism. 6 full cross-tradition clusters substantively articulate this shape (Johar/Sufi + Sahely-Maturana + Post-Marxist critical theory genealogically-aggregated + Postcolonial critique + State-legibility/bureaucratic-form critique + Galtung Norwegian-peace-research). Three layer-instantiations per Sahely W4.2: Security Trap (security-policy layer) + Growth Illusion (economic layer) + Cancer Stage of Capitalism (civilizational scale, McMurtry 1999 canonical). Relationship to substitution-trap mode is shape-of (golden-calf-trap is shape-of substitution-trap), not equivalent-to.

- **`recursive-audit-method`** — operational discipline for surfacing captured distinctions by applying a viability-grammar's own question to itself. 2 full cross-tradition clusters supporting the method specifically (Sahely-Maturana school + Johar/Sufi internal-recursive-audit). Composes with existing Spore canon-method discipline feedback memories (`feedback_audit_then_propose.md` + `feedback_intake_to_vocab_admission_program.md` + `feedback_parsimony_elegance_dual_discipline.md`) as canonical Spore-side anchor for the discipline they collectively enact.

ADR-0085 also admits a second trap-shape variant via framing-note layer (substrate-resonance documented; not slug admission): **Sacralized Trojan-Horse class** (preservation-via-reverence shape) — the corrective distinction is preserved and revered specifically to prevent its critique. Primary substrate W3.1 sacred-memory-transformation; folded as Trojan-Horse-class trap subtype per §17.8 (2). See `docs/research/connections/canon-framing-recursive-audit-method.md` for full trap-shape taxonomy + recursive-audit-method canon-method articulation + folded substrate documentation (C11 R-Civilization-extended-niche + C14 R-Externalization-as-deferred-collapse + C15 R-Sacred-memory-transformation as Trojan-Horse-class).
...
- **ADR-0085 (trap-shape-vocab-and-recursive-audit-method)**: extends substitution-trap mode with canon-legible trap-shape vocabulary (`golden-calf-trap` simple-substitution + Sacralized Trojan-Horse preservation-via-reverence as framing-note shape-variant) + recursive-audit-method canonical anchor for surfacing-discipline + dedicated framing-note `canon-framing-recursive-audit-method.md` with full trap-shape taxonomy + folded substrate documentation (C11 + C14 + C15). ADR-0048 file content extended via narrative-prose per extend-via-prose precedent (ADR-0050 / ADR-0077 pattern); frontmatter status / decision PRESERVED unchanged.
