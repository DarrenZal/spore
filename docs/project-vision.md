...
- **Membrane** — the semi-permeable interface between holon and field, and between holons. Membranes filter, translate, authorize, and attest. Spore's strongest-supported primitive across coordination traditions; the near-universal name for what separates operational closure from environmental coupling.
    ...
    - *Production-mode scope* (ADR-0062; capstone §5 R-Mem-1 residue): Spore's Membrane primitive spans two operationally-distinct production-modes, and the reading that applies depends on the instantiation context — canon commits to both as operationally valid rather than privileging one as universal. **Self-produced membranes** are constituted by ongoing commoning, stewardship, or care labor: the boundary's existence and character are outcomes of the community's continuing practice rather than of prior declaration (BKC commons-membrane produced by stewardship rotations and ontology-curation; IC memory-stewardship-membrane produced by attribution and preservation labor; Octo agent-membrane shaped by knowledge-gardening). This reading aligns with Maturana-Varela autopoiesis (the system's processes continuously regenerate the network that produced them, including its boundary) and Bollier-Helfrich commons-as-self-produced (the semi-permeable commons membrane is the ongoing commoning activity). **Text-authoritative membranes** are established declaratively: the boundary is specified by the artifact's text and is canonical by construction rather than by labor-production (federation-protocol-version membranes defined in the spec-DAG via `depends_on`; governance-artifact scope declared in vision/agreement/policy text; spec-DAG node membranes structurally defined by frontmatter). This reading aligns with ADR-0041's text-authoritative discipline and Ostrom's descriptive boundary analysis. The two modes coexist in a single Membrane primitive; specific Membrane instantiations are evaluated for which mode is load-bearing. Mixed cases (PM `pm:CommitmentBundle` boundary declared at formation and maintained by ongoing attestation; federation-member-membership established by spec but sustained by compliance work) are canonically legible as combinations rather than as residual ambiguity. ADR-0062 declined to admit `membrane-as-self-produced` as a property-on-primitive or as a derived glossary slug — the earning-test did not jointly pass (Q-a marginal: names a production-mode rather than a new coordination operation; Q-b partial: two genuine autopoiesis-active cases with three clear text-authoritative counterexamples) — and chose instead to scope the existing Membrane primitive. Field-Membrane internal-consistency with ADR-0046 (Field as Ostromian-constitutive of action arenas) is preserved: Field and Membrane are distinct primitives with distinct production-modes, and honest cross-primitive asymmetry is compatible with Spore's scoped-grammar stance.
...
- **Frontmatter on docs** — a side-effect of working with the governance-memory pattern, which tracks lifecycle state and dependencies across governance artifacts. Frontmatter is a convention, not a standalone surface — adding it is how a doc participates in governance-memory, not a separate adoption act.

These surfaces are independent. One can be adopted without the others.

**One typical path (recommendation, not requirement)**: add frontmatter to governance artifacts first (low cost, no infrastructure), then stand up a sensor node and declare edge permissions when cross-project queries become useful, then add profile declarations when implementing specific patterns. This sequence has worked for early adopters; it is not mandatory, and projects with different coordination goals will reasonably choose different orderings or subsets.
